...
import 'package:hive_flutter_database/network/get_request/get_album_network.dart';
...
class GetAlbumProvider extends ChangeNotifier {
  ...
  Future<void> saveOfflineData(List<Album> album) async {
    try {
      ...
      // Decode EncryptionKey to be used in encryptionCipher

      var encryptedBox = await Hive.openBox("album",encryptionCipher: HiveAesCipher(encryptionKey));
      albumList = album;

      encryptedBox.put(200, album);
      encryptedBox.close();
      //always close box if you don't using it
    } catch (e) {
      log(e.toString());
    }
  }
  ...
  Future<void> showOfflineList() async {
    ...
    var encryptedBox = await Hive.openBox("album",encryptionCipher: HiveAesCipher(encryptionKey));
    albumHiveList = await encryptedBox.get(200, defaultValue: []);
